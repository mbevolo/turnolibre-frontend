document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function cargarDatosUsuario() {
        try {
            const res = await fetch(`http://localhost:3000/usuario/${emailUsuario}`);
    ...
    async function cargarReservas() {
        try {
            const res = await fetch(`http://localhost:3000/reservas-usuario/${emailUsuario}`);
            ...
            reservasFuturas.forEach(r => {
                ...
                const deporteCapitalizado = r.deporte.charAt(0).toUpperCase() + r.deporte.slice(1);
